...
def load_residential_db(index_dir: str, model_name: str):
    embeddings = HuggingFaceEmbeddings(model_name=model_name)
    db = FAISS.load_local(index_dir, embeddings=embeddings, allow_dangerous_deserialization=True)
    return db, embeddings
    

def query_ollama_residential_property(db: FAISS, question: str, k: int = 3, model: str = "qwen2.5:3b") -> str:
    # retrieve context
